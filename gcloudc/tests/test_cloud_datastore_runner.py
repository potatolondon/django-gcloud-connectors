...
class CloudDatastoreRunnerTest(TestCase):
    ...
    def test_check_gcloud_components(self):
        class MockProcess:
            stdout = json.dumps(
                [{'id': cp, 'current_version_string': '0.1'} for cp in list(_REQUIRED_COMPONENTS)[:-1]]
            )

        # We mock _start_emulator as we don't want to get that far in execute()
        with patch('gcloudc.commands.management.commands.CloudDatastoreRunner._start_emulator',
